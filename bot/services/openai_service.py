...
_FOOD_PHOTO_SYSTEM_PROMPT = (
    ...
    "double-check you're reading the protein row specifically, not fiber, sugar, or sodium. If the label "
    "shows both a 'per 100g' and a 'per serving' column, the per-serving numbers should be roughly "
    "proportional to per-100g scaled by the serving size — use that as a sanity check, and if a value you "
    "read doesn't roughly fit that ratio, re-examine it rather than reporting it as-is. "
